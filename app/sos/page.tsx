  ...
  return (
    <main className="min-h-screen bg-[#F6F7F9] text-[#0D1B2A] p-8">
      <div className="max-w-5xl mx-auto">
        ...
        <p className="text-gray-500 mb-8">Emergency events auto-refresh every 30 seconds</p>
        {loading ? <p className="text-gray-500">Loading…</p> : events.length === 0 ? <p className="text-gray-500">No SOS events recorded.</p> : (
          <div className="flex flex-col gap-4">
            {events.map(e => (
              <div key={e.id} className="bg-white rounded-xl p-6 border shadow-sm" style={{ borderColor: e.status === 'OPEN' ? '#ef4444' : '#e5e7eb' }}>
                <div className="flex justify-between items-start mb-3">
                  <div>
                    ...
                    <p className="text-gray-500 text-sm">{new Date(e.created_at).toLocaleString()}</p>
                    {e.location && <p className="text-gray-500 text-sm">📍 {e.location.lat?.toFixed(5)}, {e.location.lng?.toFixed(5)}</p>}
                    {e.trip_id && <p className="text-gray-500 text-sm">Trip: {e.trip_id}</p>}
                  </div>
